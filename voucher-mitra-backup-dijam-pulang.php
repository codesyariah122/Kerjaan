...
add_action('woocommerce_cart_calculate_fees', function ($cart) {
    ...
    foreach ($cart->get_cart() as $item) {
        ...
        if ($is_deposit) {
            // Jika produk deposit aktif → ambil rate dari meta (default 30%)
            $deposit_rate = (float) get_post_meta($variation_id, '_yith_wcdp_deposit_rate', true);
            if ($deposit_rate <= 0) $deposit_rate = 30;
        } else {
            // Kalau bukan produk deposit → DP & Pelunasan 50/50
            $deposit_rate = 50;
        }

        // Hitung DP & Pelunasan per produk
...
ini backup terbaru di jam 17:00 saat jam pulang 
...
//         // 💡 Hitung diskon Mitra berdasarkan total harga produk
//         $discount = ($mitra_discount_percent / 100) * $subtotal;
//         $discounted_total = $subtotal - $discount;

//         // --- ⬇️ Tambahkan blok ini di sini ---
//         if ($is_deposit && $is_override) {
//             $dp = ($deposit_rate / 100) * $discounted_total;
//             $pelunasan = $discounted_total - $dp;
//         } else {
//             $dp = 0;
...
//         // --- ⬆️ sampai sini ---
//         if ($is_deposit) {
//             // Jika produk deposit aktif → ambil rate dari meta (default 50%)
//             $deposit_rate = (float) get_post_meta($variation_id, '_yith_wcdp_deposit_rate', true);
//             if ($deposit_rate <= 0) $deposit_rate = 50; // default DP 50%
//         } else {
//             $deposit_rate = 50;
//         }

//         // ✅ Diskon diterapkan ke total harga produk (bukan hanya DP)
//         $dp = ($deposit_rate / 100) * $discounted_total;
//         $pelunasan = $discounted_total - $dp;
...
//     // Tambahkan potongan Diskon Mitra ke total cart
//     $cart->add_fee(__('Diskon Mitra (' . $mitra_discount_percent . '%)', 'woocommerce'), -$total_discount, false);
// }, 25);
