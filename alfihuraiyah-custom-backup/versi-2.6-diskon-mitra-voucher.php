...
/**
 * Plugin Name: Override Voucher Mitra (v2.5 Hybrid Smart Display)
 * Description: Versi kombinasi 2.3 + 2.4. Hitungan akurat mengikuti skema voucher -> harga mitra -> DP/pelunasan. Menampilkan info dinamis di single product: jika belum apply kupon, tampil pesan informatif agar tidak menyesatkan.
 * Version: 2.5
 * Author: Puji Ermanto | UCOK AKA (hybrid fix)
...
/**
 * Ambil persentase diskon voucher mitra dari WooCommerce coupon aktif.
 * Default = 0 (tidak potong apapun).
 */
...
/**
 * Ambil persentase harga mitra dari product meta (_mitra_price_percent).
 * Default 50%.
 */
...
/* =====================================================
   2️⃣ Simpan Payment Type (Full/Deposit)
===================================================== */
...
/* =====================================================
   3️⃣ Hitung Diskon Mitra di Cart
===================================================== */
...
/* =====================================================
   5️⃣ Smart Display di Single Product
===================================================== */
add_filter('woocommerce_get_price_html', function ($price_html, $product) {
    ...
    if ($voucher_percent > 0) {
        // Jika ada kupon aktif, tampilkan harga real
        ...
        // Jika belum apply kupon, tampilkan simulasi + pesan
        $harga_mitra = $original * ($mitra_percent / 100);
        return sprintf(
            '<del>%s</del> <ins>%s</ins><br><small style="color:#999;">Harga Mitra simulasi (%s%%). Aktifkan voucher mitra untuk melihat harga setelah diskon.</small>',
            wc_price($original),
            wc_price($harga_mitra),
            esc_html($mitra_percent)
        );
...
/* =====================================================
   6️⃣ Tampilan Full vs Deposit di Single Product
===================================================== */
    ...
    <div class="yith-wcdp-single-add-to-cart-fields" data-deposit-type="rate" data-deposit-amount="<?php echo esc_attr($dp); ?>">
        <label class="full">
            ...
            <span class="label">
                ...
                <?php else : ?>
                    <small class="yith-wcdp-note" style="color:#888;">Harga Mitra simulasi (belum ada voucher aktif)</small>
                <?php endif; ?>
            </span>
        </label>
        <label class="deposit">
...
/* =====================================================
   7️⃣ Paksa Override DP untuk Produk Mitra
===================================================== */
add_filter('yith_wcdp_get_deposit_amount', function ($deposit_amount, $product_id) {
    ...
    $voucher_percent = ovm_get_applied_voucher_percent();
    $mitra_percent = ovm_get_mitra_price_percent($product_id);
    $product = wc_get_product($product_id);
    if (! $product) return $deposit_amount;

    $original = (float) $product->get_regular_price();
    if ($original <= 0) return $deposit_amount;

    // rumus: harga mitra setelah diskon voucher
    $after_voucher = $original - ($original * ($voucher_percent / 100));
    $harga_mitra = $after_voucher * ($mitra_percent / 100);
    $dp_mitra = $harga_mitra * 0.5;

    return $dp_mitra; // paksa override
}, 999, 2);


/* =====================================================
   8️⃣ Universal Fallback — Tampilkan Skema Full / Deposit
   (selalu muncul di bawah tombol “Tambah ke Keranjang”)
===================================================== */
        ...
        <label style="display:block;margin-bottom:8px;">
            ...
            <?php if ($voucher_percent > 0): ?>
                <small style="color:#3A0BF4;">Harga Mitra setelah diskon
                    <?php echo esc_html($voucher_percent); ?>%
                    & potongan
                    <?php echo esc_html($mitra_percent); ?>%</small>
            <?php else: ?>
                <small style="color:#888;">Harga simulasi (belum pakai voucher)</small>
            <?php endif; ?>
        </label>

        <label style="display:block;margin-bottom:8px;">
            <input type="radio" name="payment_type" value="deposit">
            <strong>Bayar DP:</strong> <span><?php echo wc_price($dp); ?></span><br>
            <small>Sisa pelunasan
                (<?php echo wc_price($pelunasan); ?>)
                sebelum
                <?php echo date_i18n('F j, Y', strtotime('+10 days')); ?>
            </small>
        </label>
    </div>
<?php
...
/* =====================================================
   9️⃣ DOM Injector — Tambahkan Pilihan Full / DP
   Langsung ke Form Produk (tanpa bergantung pada Hook)
===================================================== */
...
<?php
    ...
    // sisipkan sebelum tag </form> pertama (biasanya form add to cart)
