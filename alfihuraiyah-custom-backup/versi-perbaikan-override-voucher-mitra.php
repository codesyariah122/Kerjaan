...
add_action('woocommerce_cart_calculate_fees', function ($cart) {
    ...
    // Jika tidak ada kupon, fallback 0%
    if ($voucher_percent <= 0) $voucher_percent = 0;

    // 💰 Hitung diskon dari total harga produk normal
    $discount_amount = ($voucher_percent / 100) * $original_total;
    $total_after_discount = $original_total - $discount_amount;

    // 💳 Hitung DP dan pelunasan (50%-50%)
    $dp = $total_after_discount * 0.5;
    $pelunasan = $total_after_discount * 0.5;

    // Simpan ke session agar ditampilkan di cart/checkout
...
add_action('woocommerce_cart_totals_before_order_total', function () {
    ...
    echo '<tr class="fee cart-mitra-diskon">
        <th style="color:#3A0BF4;">Diskon Mitra (25%)</th>
        <td style="color:#3A0BF4;">–' . wc_price($total_discount) . '</td>
    </tr>';

    echo '<tr class="fee cart-mitra-harga">
        <th style="color:#3A0BF4;">Harga Mitra (50%)</th>
        <td style="color:#3A0BF4;">' . wc_price($dp * 2) . '</td>
    </tr>';

    echo '<tr class="fee cart-mitra-dp">
        <th style="color:#3A0BF4;">DP (50%)</th>
        <td style="color:#3A0BF4;">' . wc_price($dp) . '</td>
    </tr>';

    echo '<tr class="fee cart-mitra-pelunasan">
        <th style="color:#3A0BF4;">Pelunasan (50%)</th>
        <td style="color:#3A0BF4;">' . wc_price($pelunasan) . '</td>
    </tr>';
});
