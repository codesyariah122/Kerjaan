/**
 * Plugin Name: Override Voucher Mitra (v3.7 Alfihuraiyah Fix)
 * Description: Override total YITH Deposit dan tampilkan versi Mitra (Bahasa Indonesia). Kompatibel penuh dengan struktur tema Alfihuraiyah.
 * Version: 3.7
		...
		<label style="display:block;margin-bottom:8px;">
			...
			<?php if ( $voucher_percent > 0 ) : ?>
				<small style="color:#3A0BF4;">
					Setelah diskon voucher <?php echo esc_html( $voucher_percent ); ?>%
					dan potongan mitra <?php echo esc_html( $mitra_percent ); ?>%
				</small>
			<?php else : ?>
...
/* =====================================================
   🪄 DOM INJECTOR — Spesifik untuk tema Alfihuraiyah
===================================================== */
add_action('wp_footer', function () {
    ...
    const insertMitraBlock = () => {
        const target = document.querySelector('.woocommerce-variation-add-to-cart');
        if (target && !document.querySelector('.mitra-payment-options')) {
            const wrap = document.createElement('div');
            wrap.innerHTML = <?php echo $escaped_html; ?>;
            target.insertAdjacentElement('afterend', wrap);
            console.log('✅ Blok Pembayaran Mitra disisipkan (observer).');
        }
    };
    ...
    // Observer untuk dynamic load variasi
    const observer = new MutationObserver(insertMitraBlock);
